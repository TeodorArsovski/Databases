BEGIN
  ...
  EXECUTE IMMEDIATE 'DROP TABLE MINERAL           CASCADE CONSTRAINTS'; 
  
  EXECUTE IMMEDIATE 'DROP TABLE BOOK              CASCADE CONSTRAINTS';
  
  EXECUTE IMMEDIATE 'DROP TABLE DONATABLE         CASCADE CONSTRAINTS';

  EXECUTE IMMEDIATE 'DROP TABLE MUSEUM            CASCADE CONSTRAINTS';
  
  EXECUTE IMMEDIATE 'DROP TABLE RESIDENTEVENT     CASCADE CONSTRAINTS';
  
  EXECUTE IMMEDIATE 'DROP TABLE HAPPEN            CASCADE CONSTRAINTS';
  
  EXECUTE IMMEDIATE 'DROP TABLE EVENT             CASCADE CONSTRAINTS';
  
  EXECUTE IMMEDIATE 'DROP TABLE SELL              CASCADE CONSTRAINTS';
  
  EXECUTE IMMEDIATE 'DROP TABLE PARTICIPATE       CASCADE CONSTRAINTS';

  EXECUTE IMMEDIATE 'DROP TABLE SPECIALITEM       CASCADE CONSTRAINTS';

  EXECUTE IMMEDIATE 'DROP TABLE COMPOSE           CASCADE CONSTRAINTS';
  
  EXECUTE IMMEDIATE 'DROP TABLE ACTIVITY          CASCADE CONSTRAINTS';
  
  EXECUTE IMMEDIATE 'DROP TABLE FESTIVITY         CASCADE CONSTRAINTS';
  ...
  EXECUTE IMMEDIATE 'DROP TABLE LETTER           CASCADE CONSTRAINTS';
   --PLAYERMISSION
  EXECUTE IMMEDIATE 'DROP TABLE PLAYER_MISSION CASCADE CONSTRAINTS';
  --MISSION
  ...
  EXECUTE IMMEDIATE 'DROP SEQUENCE SEQ_RELATIONSHIP_ID';

  EXECUTE IMMEDIATE 'DROP SEQUENCE SEQ_PLACE_ID';
...
CREATE TABLE DIALOGUE (
  ...
  ID_INHABITANT  NUMBER        NOT NULL,
  CONTENT        VARCHAR2(200)          NOT NULL,
  SKIPPABLE      VARCHAR2(5)   NOT NULL,

  CONSTRAINT FK_DIALOGUE_INHABITANT
    FOREIGN KEY (ID_INHABITANT)
...
CREATE TABLE RECEIVE (
  ...
  ID_PLAYER      NUMBER,
  ID_LETTER      NUMBER,
  EMOTION        VARCHAR2(200),
  PRIMARY KEY (ID_INHABITANT, ID_PLAYER, ID_LETTER),
...
CREATE TABLE FESTIVITY (
  ID_FESTIVITY   NUMBER PRIMARY KEY,
  NAME_FESTIVITY           VARCHAR2(100),
  IS_COMPLETED   VARCHAR2(5) CHECK (IS_COMPLETED IN ('TRUE', 'FALSE'))
);

-- ACTIVITY
CREATE TABLE ACTIVITY (
  ID_ACTIVITY   NUMBER PRIMARY KEY,
  NAME_ACTIVITY          VARCHAR2(100),
  DESCRIPTION   VARCHAR2(250),
  DURATION      NUMBER,
  ID_FESTIVITY  NUMBER,
  CONSTRAINT FK_ACTIVITY_FESTIVITY FOREIGN KEY (ID_FESTIVITY)
    REFERENCES FESTIVITY(ID_FESTIVITY)
);

-- COMPOSE (many-to-many between FESTIVITY and ACTIVITY)
CREATE TABLE COMPOSE (
  ID_ACTIVITY   NUMBER,
  ID_FESTIVITY  NUMBER,
  PRIMARY KEY (ID_ACTIVITY, ID_FESTIVITY),
  CONSTRAINT FK_COMPOSE_ACTIVITY FOREIGN KEY (ID_ACTIVITY)
    REFERENCES ACTIVITY(ID_ACTIVITY),
  CONSTRAINT FK_COMPOSE_FESTIVITY FOREIGN KEY (ID_FESTIVITY)
    REFERENCES FESTIVITY(ID_FESTIVITY)
);

-- SPECIAL ITEM
CREATE TABLE SPECIALITEM (
  ID_SPECIALITEM  NUMBER PRIMARY KEY
  -- Optionally link to ITEM table with FK if ID_SPECIALITEM = ID_ITEM
);
...
CREATE TABLE PARTICIPATE (
  ID_ACTIVITY     NUMBER,
  ID_PLAYER       NUMBER,
  ID_SPECIALITEM  NUMBER,
  PRIMARY KEY (ID_ACTIVITY, ID_PLAYER, ID_SPECIALITEM),
  CONSTRAINT FK_PARTICIPATE_ACTIVITY FOREIGN KEY (ID_ACTIVITY)
    REFERENCES ACTIVITY(ID_ACTIVITY),
  CONSTRAINT FK_PARTICIPATE_PLAYER FOREIGN KEY (ID_PLAYER)
    REFERENCES PLAYER(ID_PLAYER),
  CONSTRAINT FK_PARTICIPATE_SPECIALITEM FOREIGN KEY (ID_SPECIALITEM)
    REFERENCES SPECIALITEM(ID_SPECIALITEM)
);

-- SELL (Shop sells during a festivity)
CREATE TABLE SELL (
  ID_FESTIVITY  NUMBER,
  ID_SHOP       NUMBER,
  DISCOUNT      NUMBER(5,2),
  PRIMARY KEY (ID_FESTIVITY, ID_SHOP),
  CONSTRAINT FK_SELL_FESTIVITY FOREIGN KEY (ID_FESTIVITY)
    REFERENCES FESTIVITY(ID_FESTIVITY),
  CONSTRAINT FK_SELL_SHOP FOREIGN KEY (ID_SHOP)
    REFERENCES SHOP(ID_SHOP)
);
...
CREATE TABLE EVENT (
  ...
  DESCRIPTION  VARCHAR2(250),
  ID_PLACE     NUMBER,
  CONSTRAINT FK_EVENT_PLACE FOREIGN KEY (ID_PLACE)
    REFERENCES PLACE(ID_PLACE)
);

-- HAPPEN (Event occurs at a Place)
CREATE TABLE HAPPEN (
  ID_EVENT  NUMBER,
  ID_PLACE  NUMBER,
  PRIMARY KEY (ID_EVENT, ID_PLACE),
  CONSTRAINT FK_HAPPEN_EVENT FOREIGN KEY (ID_EVENT)
    REFERENCES EVENT(ID_EVENT),
  CONSTRAINT FK_HAPPEN_PLACE FOREIGN KEY (ID_PLACE)
    REFERENCES PLACE(ID_PLACE)
);

-- RESIDENTEVENT (Resident + Player attend a special Event)
CREATE TABLE RESIDENTEVENT (
  ID_RESIDENTEVENT  NUMBER PRIMARY KEY,
  ID_RESIDENT       NUMBER,
  ID_PLAYER         NUMBER,
  CONSTRAINT FK_RESIDENTEVENT_INHABITANT FOREIGN KEY (ID_RESIDENT)
    REFERENCES INHABITANT(ID_INHABITANT),
  CONSTRAINT FK_RESIDENTEVENT_PLAYER FOREIGN KEY (ID_PLAYER)
    REFERENCES PLAYER(ID_PLAYER)
);

-- MUSEUM
CREATE TABLE MUSEUM (
  ID_MUSEUM     NUMBER PRIMARY KEY,
  COMPLETION    NUMBER(5,2), -- percentage of completion
  ID_VILLAGE    NUMBER,
  CONSTRAINT FK_MUSEUM_VILLAGE FOREIGN KEY (ID_VILLAGE)
    REFERENCES VILLAGE(ID_VILLAGE)
);

-- DONATABLE (superclass)
CREATE TABLE DONATABLE (
  ID_DONATABLE  NUMBER PRIMARY KEY
);

-- BOOK (subclass of DONATABLE)
CREATE TABLE BOOK (
  ID_BOOK     NUMBER PRIMARY KEY,
  TITLE       VARCHAR2(200),
  NUM_PAGES   NUMBER,
  CONSTRAINT FK_BOOK_DONATABLE FOREIGN KEY (ID_BOOK)
    REFERENCES DONATABLE(ID_DONATABLE)
);

-- MINERAL (subclass of DONATABLE)
CREATE TABLE MINERAL (
  ID_MINERAL  NUMBER PRIMARY KEY,
  QUALITY     VARCHAR2(100),
  RARITY      VARCHAR2(100),
  CONSTRAINT FK_MINERAL_DONATABLE FOREIGN KEY (ID_MINERAL)
    REFERENCES DONATABLE(ID_DONATABLE)
);

-- DONATION (n-m relationship with extra attribute: condition_object)
CREATE TABLE DONATION (
  ID_DONATABLE  NUMBER,
  ID_MUSEUM     NUMBER,
  ID_PLAYER     NUMBER,
  CONDITION_OBJECT VARCHAR2(100),
  PRIMARY KEY (ID_DONATABLE, ID_MUSEUM, ID_PLAYER),
  CONSTRAINT FK_DONATION_DONATABLE FOREIGN KEY (ID_DONATABLE)
    REFERENCES DONATABLE(ID_DONATABLE),
  CONSTRAINT FK_DONATION_MUSEUM FOREIGN KEY (ID_MUSEUM)
    REFERENCES MUSEUM(ID_MUSEUM),
  CONSTRAINT FK_DONATION_PLAYER FOREIGN KEY (ID_PLAYER)
    REFERENCES PLAYER(ID_PLAYER)
);
