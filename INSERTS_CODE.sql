...
-- Follow table
INSERT INTO FOLLOW (ID_SHOP, ID_SCHEDULE)
SELECT DISTINCT
       iv.ID_SHOP,
       sch.ID_SCHEDULE
FROM INVENTORIES iv
...
-- Insert unique FESTIVITY records
INSERT INTO FESTIVITY (
    ID_FESTIVITY,
    NAME_FESTIVITY,
    IS_COMPLETED
)
SELECT DISTINCT
    TO_NUMBER(ID) AS ID_FESTIVITY,
    DESCRIPTION   AS NAME_FESTIVITY,
    'FALSE'       AS IS_COMPLETED
FROM (
    SELECT ID, DESCRIPTION FROM FESTIVITY_PERSONS
    UNION
    SELECT ID, DESCRIPTION FROM FESTIVITY_SHOPS
) f
WHERE ID IS NOT NULL AND DESCRIPTION IS NOT NULL;


-- Insert into ACTIVITY using ROW_NUMBER to deduplicate
...
SELECT
  TO_NUMBER(a.ID_ACTIVITY),
  a.SPECIAL_FACTOR AS NAME_ACTIVITY,
  a.DESCRIPTION,
  TO_NUMBER(a.DURATION),
  TO_NUMBER(a.ID_EVENT)
FROM (
  SELECT a.*,
         ROW_NUMBER() OVER (PARTITION BY a.ID_ACTIVITY ORDER BY ROWNUM) AS rn
  FROM ACTIVITIES a
  WHERE a.ID_ACTIVITY IS NOT NULL
    AND a.DESCRIPTION IS NOT NULL
    AND a.DURATION IS NOT NULL
    AND a.ID_EVENT IS NOT NULL
) a
WHERE a.rn = 1
  AND TO_NUMBER(a.ID_ACTIVITY) NOT IN (
    SELECT ID_ACTIVITY FROM ACTIVITY
  );
...
-- Insert into COMPOSE (Activity-Festivity relationship)
INSERT INTO COMPOSE (
    ID_ACTIVITY,
    ID_FESTIVITY
)
SELECT DISTINCT
    TO_NUMBER(ID_ACTIVITY),
    TO_NUMBER(ID_EVENT) AS ID_FESTIVITY
FROM ACTIVITIES
WHERE ID_ACTIVITY IS NOT NULL
  AND ID_EVENT IS NOT NULL;


-- Insert into SPECIALITEM table
INSERT INTO SPECIALITEM (
    ID_SPECIALITEM
)
SELECT DISTINCT
    TO_NUMBER(REWARD_ITEM)
FROM ACTIVITIES
WHERE REWARD_ITEM IS NOT NULL;


-- Safe insert into PARTICIPATE with deduplication
...
WHERE TO_NUMBER(a.ID_ACTIVITY) IS NOT NULL
  AND TO_NUMBER(a.REWARD_ITEM) IS NOT NULL
  AND TO_NUMBER(p.ID) IS NOT NULL
  AND NOT EXISTS (
    SELECT 1
    FROM PARTICIPATE part
    WHERE part.ID_ACTIVITY = TO_NUMBER(a.ID_ACTIVITY)
      AND part.ID_PLAYER = TO_NUMBER(p.ID)
      AND part.ID_SPECIALITEM = TO_NUMBER(a.REWARD_ITEM)
  );
...
SELECT
    TO_NUMBER(s.ID),
    TO_NUMBER(s.ID_SHOP),
    TO_NUMBER(s.PERCENTAGE)
FROM (
    SELECT s.ID, s.ID_SHOP, s.PERCENTAGE,
           ROW_NUMBER() OVER (PARTITION BY s.ID, s.ID_SHOP ORDER BY ROWNUM) AS rn
    FROM FESTIVITY_SHOPS s
    WHERE s.ID IS NOT NULL
      AND s.ID_SHOP IS NOT NULL
...
WHERE s.rn = 1
  AND NOT EXISTS (
    SELECT 1
    FROM SELL se
    WHERE se.ID_FESTIVITY = TO_NUMBER(s.ID)
      AND se.ID_SHOP = TO_NUMBER(s.ID_SHOP)
  );
...
SELECT DISTINCT
    ...
    (
        SELECT p.ID_PLACE
        FROM PLACE p
        WHERE p.LOCATION_NAME = e.LOCATION_NAME
          AND p.COORDINATES = e.COORDENATES
          AND ROWNUM = 1
    ) AS ID_PLACE
FROM EVENTS e
WHERE e.ID IS NOT NULL
  ...
  AND e.COORDENATES IS NOT NULL;



-- Insert into HAPPEN table
INSERT INTO HAPPEN (
    ID_EVENT,
    ID_PLACE
)
SELECT DISTINCT
    TO_NUMBER(e.ID) AS ID_EVENT,
    p.ID_PLACE
FROM EVENTS e
JOIN EVENT ev ON TO_NUMBER(e.ID) = ev.ID_EVENT
JOIN PLACE p
  ON p.LOCATION_NAME = e.LOCATION_NAME
 AND p.COORDINATES = e.COORDENATES
WHERE e.ID IS NOT NULL
  AND e.LOCATION_NAME IS NOT NULL
  AND e.COORDENATES IS NOT NULL
  AND NOT EXISTS (
    SELECT 1
    FROM HAPPEN h
    WHERE h.ID_EVENT = TO_NUMBER(e.ID)
      AND h.ID_PLACE = p.ID_PLACE
  );



-- Insert into RESIDENTEVENT table
INSERT INTO RESIDENTEVENT (
    ID_RESIDENTEVENT,
    ID_RESIDENT,
    ID_PLAYER
)
SELECT
    ROWNUM + 1000 AS ID_RESIDENTEVENT,
    TO_NUMBER(e.ID_RESIDENT),
    (
        SELECT TO_NUMBER(p.ID)
        FROM PLAYER_DATA p
        WHERE ROWNUM = 1
    ) AS ID_PLAYER
FROM EVENTS e
WHERE e.ID_RESIDENT IS NOT NULL;
...
SELECT DISTINCT
    TO_NUMBER(m.ID_MUSEUM),
    100 - TO_NUMBER(m.PERCENTAGE_REMAINING),
    (
        SELECT v.ID_VILLAGE
        FROM VILLAGE v
        WHERE v.NAME = m.VILLAGE_NAME
        AND ROWNUM = 1
    ) AS ID_VILLAGE
FROM MUSEUM_REGISTRY m
WHERE m.ID_MUSEUM IS NOT NULL
  AND m.VILLAGE_NAME IS NOT NULL
  AND m.PERCENTAGE_REMAINING IS NOT NULL;



-- Insert into DONATABLE
INSERT INTO DONATABLE (
    ID_DONATABLE
)
SELECT DISTINCT
    TO_NUMBER(DONABLE_ITEM)
FROM MUSEUM_REGISTRY
WHERE DONABLE_ITEM IS NOT NULL;


-- Insert into BOOK (only where TITLE and PAGES are present)
INSERT INTO BOOK (
    ID_BOOK,
    TITLE,
    NUM_PAGES
...
SELECT DISTINCT
    TO_NUMBER(DONABLE_ITEM) AS ID_BOOK,
    TITLE,
    TO_NUMBER(PAGES) AS NUM_PAGES
FROM MUSEUM_REGISTRY
WHERE DONABLE_ITEM IS NOT NULL
...
-- Insert into MINERAL (only where QUALITY and RARITY are present)
INSERT INTO MINERAL (
    ID_MINERAL,
    QUALITY,
    RARITY
...
SELECT DISTINCT
    TO_NUMBER(DONABLE_ITEM) AS ID_MINERAL,
    QUALITY,
    TO_CHAR(RARITY) AS RARITY
FROM MUSEUM_REGISTRY
WHERE DONABLE_ITEM IS NOT NULL
  AND QUALITY IS NOT NULL
...
SELECT DISTINCT
    TO_NUMBER(DONABLE_ITEM),
    TO_NUMBER(ID_MUSEUM),
    TO_NUMBER(REWARDED_PLAYER),
    STATUS AS CONDITION_OBJECT
FROM MUSEUM_REGISTRY
WHERE DONABLE_ITEM IS NOT NULL
  AND ID_MUSEUM IS NOT NULL
  AND REWARDED_PLAYER IS NOT NULL
